...
class ComicArchive:
    ...
    def is_writable_for_style(self, data_style):
        ...
        if self.is_rar() and data_style == MetaDataStyle.CBI:
            return False

        return self.is_writable()

    def seems_to_be_a_comic_archive(self):
        if (self.is_zip() or self.is_rar()) and (self.get_number_of_pages() > 0):
            return True
    ...
    def get_page(self, index):
        ...
        if filename is not None:
            try:
                ...
                err_msg = "Error reading in page. Substituting logo page."
